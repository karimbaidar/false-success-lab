# Changelog

## Unreleased

- Repositioned the repo as **False Success Lab**.
- Added a real repo-local architecture image.
- Rewrote the README around scanner report cards, built-in scenarios, proof
  trails, naive vs protected behavior, and the `agent-consistency` relationship.
- Updated repo rename guidance for the `false-success-lab` name.
- Expanded governance, support, contribution, and issue-template coverage for
  the broader lab scope.

## 0.3.0

- Converted the public experience from a refund-centered demo into False
  Success Lab.
- Added three entry points: built-in scenarios, local report import, and public
  GitHub repo scan.
- Added seven built-in scenario categories.
- Added report-card, proof-trail, receipt JSON, and copyable-fix UI.
